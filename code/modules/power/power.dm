...
/proc/electrocute_mob(mob/living/carbon/M as mob, var/power_source, var/obj/source, var/siemens_coeff = 1.0, var/ignore_gloves = 0)
	if(istype(M.loc,/obj/mecha))	return 0	//feckin mechs are dumb
	if(!ignore_gloves)
		//if(M.luck >= 0 && prob(M.luck+2)) return 0 //There's a small chance that RNG doesn't shock you, and this increased as you get more lucky.
	...
	if(M.luck < 0)
		shock_damage += rand(abs(M.luck)/2,abs(M.luck))
	var/drained_hp = 0
	if(ishuman(M) && ignore_gloves)
		var/mob/living/carbon/human/H = M
		drained_hp = H.electrocute_act(shock_damage, source, siemens_coeff, 1) //zzzzzzap! (ignores gloves)
	else
