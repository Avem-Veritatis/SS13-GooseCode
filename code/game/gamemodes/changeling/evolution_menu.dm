...
/obj/effect/proc_holder/changeling/evolution_menu
	name = "-Callidus Menu-" //Dashes are so it's listed before all the other abilities.
	desc = "Choose your weapons."
	dna_cost = 0
...
/obj/effect/proc_holder/changeling/evolution_menu/proc/create_menu(var/datum/changeling/changeling)
	var/dat
	dat +="<html><head><title>Callidus Menu</title></head>"
		...
		<head>
			<script type='text/javascript'>
				...
				function expand(id,name,desc,helptext,power,ownsthis){
					...
					if(!ownsthis)
					{
						body += "<a href='?src=\ref[src];P="+power+"'>Initialize</a>"
					}
					body += "</td><td align='center'>";
	...
	dat += {"
		...
		<table width='560' align='center' cellspacing='0' cellpadding='5' id='maintable'>
			<tr id='title_tr'>
				<td align='center'>
					<font size='5'><b>Callidus Menu</b></font><br>
					Hover over an ability to see more information<br>
					Current ability choices remaining: [changeling.geneticpoints]<br>
					By aquiring new identities, we gain enough power to alter and adapt.<br>
		...
		dat += {"
			...
			<tr id='data[i]' name='[i]' onClick="addToLocked('item[i]','data[i]','notice_span[i]')">
				<td align='center' bgcolor='[color]'>
					...
					>
					<b id='search[i]'>Initialize [P][ownsthis ? " - Purchased" : ((P.dna_cost > 1) ? " - Cost: [P.dna_cost]" : "")]</b>
					</a>
...
/datum/changeling/proc/purchasePower(var/mob/living/carbon/user, var/sting_name)
	...
	if(thepower == null)
		user << "This is awkward. Callidus ability purchase failed, please report this bug to a coder!"
		return

	if(has_sting(thepower))
		user << "We have already have this ability!"
		return
	...
	if(user.status_flags & FAKEDEATH)//To avoid potential exploits by buying new powers while in stasis, which clears your verblist.
		user << "We lack the energy to get new abilities right now."
		return
...
/datum/changeling/proc/lingRespec(var/mob/user)
	if(canrespec)
		user << "We have removed our powers from this form, and are now ready to readapt."
