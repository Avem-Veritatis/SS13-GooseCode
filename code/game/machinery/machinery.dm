...
/obj/machinery/proc/open_machine()
	...
	if(T)
		...
		if(passenger)
			if(passenger.client)
				passenger.client.eye = occupant
				passenger.client.perspective = MOB_PERSPECTIVE
			passenger = null
	update_icon()
	updateUsrDialog()
...
/obj/machinery/proc/auto_use_power()
	//if(locate(/obj/effect/fake_floor in get_turf(src))) //Fake areas always have power for now. //Lets see what happens when I turn this off. The tram has an area now.
	//	return 1                                          //Although... Ideally it would be pretty easy to make an APC that applies to movable areas.
