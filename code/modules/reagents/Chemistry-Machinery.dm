...
/obj/machinery/chem_dispenser/constructable
	...
	var/list/special_reagents = list(list("hydrogen", "oxygen", "silicon", "phosphorus", "sulfur", "carbon", "nitrogen"),
								...
								list("carpotoxin", "tricordrazine", "cornoil", "bicaridine", "chloralhydrate", "kelotane", "frostoil"))
...
/obj/machinery/reagentgrinder
		...
		var/list/blend_items = list (
				...
				//Firesuit (this is just to make fluoroantimonic acid difficult)
				/obj/item/clothing/suit/fire/firefighter = list("SbO" = 15),
...
/obj/machinery/reagentgrinder/proc/grind()
		...
		for (var/obj/item/clothing/suit/fire/firefighter/F in holdingitems) //Otherwise you can't grind up the suit.
				if (beaker.reagents.total_volume >= beaker.reagents.maximum_volume)
						break
				var/allowed = get_allowed_by_id(F)
				for (var/r_id in allowed)
						var/space = beaker.reagents.maximum_volume - beaker.reagents.total_volume
						var/amount = allowed[r_id]
						beaker.reagents.add_reagent(r_id,min(amount, space))
