...
/obj/machinery/alarm/proc/return_status()
	...
	var/total = environment.get_pressure()
...
"}
	...
	var/environment_pressure = environment.get_pressure()
	...
	var/co2_dangerlevel = cur_tlv.get_danger_level(environment.co2*GET_PP)
	var/co2_percent = round(environment.co2 / total * 100, 2)

	cur_tlv = TLV["plasma"]
	var/plasma_dangerlevel = cur_tlv.get_danger_level(environment.poison*GET_PP)
	var/plasma_percent = round(environment.poison / total * 100, 2)

	cur_tlv = TLV["other"]
	var/other_moles = 0.0
	other_moles += environment.sleepgas
	other_moles += environment.promethium
...
Chlorine: <span class='dl[plasma_dangerlevel]'>[plasma_percent]</span>%<br />
"}
...
/obj/machinery/alarm/process()
	...
	cur_tlv = TLV["oxygen"]
	var/oxygen_dangerlevel = cur_tlv.get_danger_level(1/environment.oxygen)

	cur_tlv = TLV["carbon dioxide"]
	var/co2_dangerlevel = cur_tlv.get_danger_level(1/environment.co2) //Zero division every tick!!!!!!!!

	cur_tlv = TLV["plasma"]
	var/plasma_dangerlevel = cur_tlv.get_danger_level(1/environment.poison)

	cur_tlv = TLV["other"]
	var/other_moles = 0.0
	other_moles += environment.promethium
	other_moles += environment.sleepgas
	var/other_dangerlevel = cur_tlv.get_danger_level(1/other_moles)
...
/obj/machinery/firealarm
	...
	var/last_process = 0
	var/buildstage = 2 // 2 = complete, 1 = no wires,  0 = circuit gone
	var/celebhacked = 0 //If it has been hacked into a party alarm. -Drake
...
/obj/machinery/firealarm/temperature_expose(datum/gas_mixture2/air, temperature, volume)
	if(src.detecting)
...
/obj/machinery/firealarm/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(panel_open)
		if (istype(W, /obj/item/device/celebhacktool))
			if(!celebhacked)
				var/obj/item/device/celebhacktool/C = W
				if(C.charges <= 0)
					user << "\red The [C] is burnt out!"
					return
				C.charges --
				var/datum/effect/effect/system/spark_spread/s = new /datum/effect/effect/system/spark_spread
				s.set_up(3, 1, src)
				s.start()
				celebhacked = 1
				user.visible_message("<span class='warning'>[user] does something suspicious to the [src]!</span>", "<span class='warning'>You overload the [src]'s alarm routines.</span>")
				if(C.charges <= 0)
					user << "\red The [C] burns out!"
			else
				user << "\red This one is already reconfigured!"
		switch(buildstage)
			if(2)
