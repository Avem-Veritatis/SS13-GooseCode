				...
				for(var/i=1,i<amount+1;i++)
					for(var/M in being_built.materials)
						if(!linked_lathe.check_mat(being_built, M))
							src.visible_message("<span class='notice'>The [src.name] beeps, \"Not enough materials to complete all operations. You have failed the machine spirit.\"</span>")
							g2g = 0
							spawn(32*i)
								linked_lathe.busy = 0
								screen = 3.1
...
/obj/machinery/computer/rdconsole/attackby(var/obj/item/weapon/D as obj, var/mob/user as mob)
	...
	if(istype(D, /obj/item/weapon/disk))
		if(t_disk || d_disk)
			user << "A disk is already loaded into the machine. Pray for forgiveness"
			return

		if(istype(D, /obj/item/weapon/disk/tech_disk)) t_disk = D
		else if (istype(D, /obj/item/weapon/disk/design_disk)) d_disk = D
		else
			user << "\red Machine cannot accept disks in that format. You have enraged the machine spirit!"
			return
		user.drop_item()
		D.loc = src
		user << "\blue You add the disk to the machine. This pleases the Omnisiah!"
	else if(istype(D, /obj/item/weapon/card/emag) && !emagged)
		playsound(src.loc, 'sound/effects/sparks4.ogg', 75, 1)
		emagged = 1
		user << "\blue You disable the security protocols. May the Omnisiah have mercy on your soul!"
	else
		..()
	src.updateUsrDialog()
	return
...
/obj/machinery/computer/rdconsole/Topic(href, href_list)
	...
	if(href_list["menu"]) //Switches menu screens. Converts a sent text string into a number. Saves a LOT of code.
		...
		if(temp_screen <= 1.1 || (3 <= temp_screen && 4.9 >= temp_screen) || src.allowed(usr) || emagged) //Unless you are making something, you need access.
			...
			usr << "Unauthorized Access. Fabricator General Notified."
		...
		if(linked_destroy)
			if(linked_destroy.busy)
				usr << "\red The Machine Spirit is busy at the moment."
					...
					if(linked_destroy)
						...
						if(!linked_destroy.hacked)
							if(!linked_destroy.loaded_item)
								usr <<"\red The destructive analyzer appears to be empty. Praise the Omnisiah!"
								screen = 1.0
								return
		...
		screen = 0.0
		if(!sync)
			usr << "\red You must connect to the network first! You heretic!"
		else
				...
				if (g2g) //Again, if input is wrong, do nothing
					...
					for(var/M in being_built.materials)
						if(!linked_imprinter.check_mat(being_built, M))
							src.visible_message("<font color='blue'>The [src.name] beeps, \"Not enough materials to complete prototype. You heretic!\"</font>")
							g2g = 0
							break
						switch(M)
							if("$glass")
		...
		griefProtection()
		var/choice = alert("Console Database Reset", "Are you sure you want to reset the console's database? Data lost cannot be recovered.", "Continue", "Cancel")
		if(choice == "Continue")
			screen = 0.0
			del(files)
...
/obj/machinery/computer/rdconsole/interact(mob/user)
	...
	switch(screen)
		...
		if(0.3)
			dat += "<div class='statusDisplay'>Constructing Object. Please Wait...</div>"
		...
		if(1.0) //Main Menu
			...
			dat += "<A href='?src=\ref[src];menu=1.1'>Current Levels</A><BR>"
			if(t_disk)
