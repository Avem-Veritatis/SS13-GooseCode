...
datum/objective/evac
	explanation_text = "Evacuate to the Luxury Shuttle."
	dangerrating = 5

datum/objective/evac/check_completion()
	var/turf/location = get_turf(owner.current.loc)
//	var/area/check_area = location.loc
	if(issilicon(owner.current))
		return 0
	...
	if(istype(location, /turf/simulated/shuttle/floor))
		return 1
	else
		return 0
...
datum/objective/protectnecron
	explanation_text = "The necrons are awakening! Do whatever you can to stop them, even if it means helping the mon'keigh."
	dangerrating = 25

datum/objective/protectnecron/check_completion()
	return 1