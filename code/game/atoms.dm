/atom
	...
	var/pressure_resistance //Useful for nothing at all, but I guess I should put this here to stop a good eight hundred compiler errors. -Drake
...
/atom/proc/assume_air(datum/gas_mixture2/giver)
	del(giver)
	return null
...
/atom/proc/Beam(atom/BeamTarget,icon_state="b_beam",icon='icons/effects/beam.dmi',time=50, maxdistance=10)
	...
	//continue to draw.
		...
		if(ismob(src)) //Only do this for a mob firer. No more broken flux arcs!
...
/atom/proc/isinsnow()
	if(istype(get_turf(src), /turf/snow))
		return 1
	else
		return 0
...
/atom/proc/obliviate() //An rather interesting proc that I am sure we can find a use for -Drake
	if(ismob(src))
		src << "\red You feel your physical form unravelling..."
	src.color = "#FFFFFF"
	animate(src, color = "#000000", time = 40)
