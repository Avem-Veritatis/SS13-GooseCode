...
/turf/proc/ReplaceWithLattice()
	...
	new /turf/simulated/floor/plating/snowy( locate(src.x, src.y, src.z) )

/turf/proc/ReplaceWithPlating()
	src.ChangeTurf(/turf/snow)
	new /turf/simulated/floor/plating/snowy( locate(src.x, src.y, src.z) )

/turf/proc/ReplaceWithTile()
	src.ChangeTurf(/turf/simulated/floor/plating/snowy)
	new /turf/simulated/floor( locate(src.x, src.y, src.z) )
...
/turf/proc/stun_creatures(mob/U = null)//Will kill people/creatures and damage mechs./N
//Useful to batch-add creatures to the list.
	for(var/mob/living/M in src)
		if(M==U)	continue//Will not harm U. Since null != M, can be excluded to kill everyone.
		spawn(0)
