...
/proc/call_shuttle_proc(var/mob/user, var/call_reason)
	...
	if(ticker.mode.name == "necron")
		user << "The console beeps 'Function unavailable at this time. We apologize for the inconvenience.'"
		return
	...
	if (ticker.mode.name == "null") // There is a serious threat we gotta move no time to give them five minutes.
