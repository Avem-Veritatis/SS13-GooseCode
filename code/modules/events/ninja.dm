...
/datum/round_event/ninja/start()
	...
	Mind.store_memory("Officially, [helping_station?"The Imperium":"Khorne"] is my employer.")
...
/datum/round_event/ninja/proc/generate_ninja_directive()
	switch(rand(1,13))
		...
		if(5)	return "We are currently negotiating with Imperial command. Prioritize saving human lives over ending them."
		...
		if(9)	return "A free agent has proposed a lucrative business deal. Implicate Imperial involvement in the operation."
	...
	switch(spideros)
		...
		if(1)
			...
			if (isnull(T))
				...
				var/datum/gas_mixture2/environment = T.get_air()
				...
				if (get_pressure)
					var/o2_level = environment.oxygen/get_pressure
					var/co2_level = environment.co2/get_pressure
					var/plasma_level = environment.promethium/get_pressure
					var/unknown_level =  1-(o2_level+co2_level+plasma_level)
					dat += "<ul>"
					dat += "<li>Oxygen: [round(o2_level*100)]%</li>"
					dat += "<li>Carbon Dioxide: [round(co2_level*100)]%</li>"
					dat += "<li>Promethium Vapors: [round(plasma_level*100)]%</li>"
					dat += "</ul>"
