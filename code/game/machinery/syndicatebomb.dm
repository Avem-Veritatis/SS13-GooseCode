...
/obj/item/weapon/bombcore/proc/detonate()
	if(adminlog)
		...
		log_game(adminlog)
	explosion(get_turf(src),5,8,15, flame_range = 16)
	qdel(src)
...
/obj/item/weapon/bombcore/training
	name = "dummy payload"
	desc = "A imperial replica of a blood pact payload. Its not intended to explode but to announce that it WOULD have exploded, then rewire itself to allow for more training."
	origin_tech = null
	var/defusals = 0
	var/attempts = 0
