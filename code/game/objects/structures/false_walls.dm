	...
	for(var/obj/structure/falsewall/W in orange(src,1))
		if(abs(src.x-W.x)-abs(src.y-W.y)) //doesn't count diagonal walls
			if(src.mineral == W.mineral)
				junction |= get_dir(src,W)
	if(mineral == "metal") //Reinforced handle their own relative_wall proc so these are explicitly regular metal walls, which have a better icon now.
		icon_state = "polar[junction]" //Unfortunately the new icon doesn't have falsewall animations but at least when the wall-door is closed it should look like a wall. //IT DOES NOW
	else
...
/obj/structure/falsewall/plasma/temperature_expose(datum/gas_mixture2/air, exposed_temperature, exposed_volume)
