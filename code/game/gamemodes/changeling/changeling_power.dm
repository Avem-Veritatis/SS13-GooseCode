...
/obj/effect/proc_holder/changeling/proc/can_sting(var/mob/user, var/mob/target)
	...
	if(req_human && !ishuman(user))
		user << "<span class='warning'>You cannot do that in this form!</span>"
		return 0
	var/datum/changeling/c = user.mind.changeling
	if(c.chem_charges<chemical_cost)
		user << "<span class='warning'>You require at least [chemical_cost] unit\s of resource to do that!</span>"
		return 0
	if(c.absorbedcount<req_dna)
		user << "<span class='warning'>You require at least [req_dna] units of biomass.</span>"
		return 0
	if(req_stat < user.stat)
		user << "<span class='warning'>You are incapacitated.</span>"
		return 0
	if((user.status_flags & FAKEDEATH) && name!="Regenerate")
		user << "<span class='warning'>You are incapacitated.</span>"
		return 0
	if(c.geneticdamage > max_genetic_damage)
		user << "<span class='warning'>Your form is still reassembling. You need time to recover first.</span>"
		return 0
	return 1
