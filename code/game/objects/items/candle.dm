...
/obj/item/candle/alt
	name = "candle"
	icon_state = "altcandle1"
	item_state = "altcandle1"
	w_class = 1

/obj/item/candle/alt/update_icon()
	var/i
	if(wax>150)
		i = 1
	else if(wax>80)
	...
	icon_state = "altcandle[i][lit ? "_lit" : ""]"

/obj/item/candle/alt/process()
	if(!lit)
		return
	wax--
	if(!wax)
		new/obj/item/trash/candle/alt(src.loc)
		if(istype(src.loc, /mob))
			var/mob/M = src.loc
			M.unEquip(src, 1) //src is being deleted anyway
		qdel(src)
