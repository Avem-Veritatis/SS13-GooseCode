/*
No longer requires air contents to do stuff!
-Drake
*/
...
/obj/machinery/atmospherics/unary/cryo_cell/proc/process_occupant()
	//if(air_contents.get_pressure() < 10)
	//	return
	if(occupant)
		...
		occupant.bodytemperature = max(occupant.bodytemperature, air_contents.temperature) // this is so ugly i'm sorry for doing it i'll fix it later i promise //APPRARENTLY NOT! Hexing this out anyway.
		*/
		occupant.bodytemperature -= 40
		occupant.bodytemperature = max(occupant.bodytemperature, 90) //Doesn't drop below 90 kelvin
...
/obj/machinery/atmospherics/unary/cryo_cell/proc/heat_gas_contents()
	if(air_contents.get_pressure() < 1)
		return
...
/obj/machinery/atmospherics/unary/cryo_cell/proc/expel_gas()
	if(air_contents.get_pressure() < 1)
		return
	var/datum/gas_mixture2/expel_gas = new
	var/remove_amount = air_contents.get_pressure() / 100
