...
/datum/game_mode/traitor
	...
	restricted_jobs = list("Cyborg", "Undercover Goose")//They are part of the AI if he is traitor so are they, they use to get double chances
...
/datum/game_mode/traitor/announce()
	world << "<B>The current game mode is - Heretic!</B>"
	world << "<B>There are Chaos traitors on the station. Do not let the heretics succeed!</B>"
...
/datum/game_mode/proc/add_law_zero(mob/living/silicon/ai/killer)
	...
	//Begin code phrase.
	killer << "The forces of Chaos have provided you with the following information on how to identify their agents:"
	if(prob(80))
		killer << "\red Code Phrase: \black [syndicate_code_phrase]"
		killer.mind.store_memory("<b>Code Phrase</b>: [syndicate_code_phrase]")
	else
		killer << "Unfortunately, Slaanesh did not provide you with a code phrase."
	if(prob(80))
		killer << "\red Code Response: \black [syndicate_code_response]"
		killer.mind.store_memory("<b>Code Response</b>: [syndicate_code_response]")
	else
		killer << "Unfortunately, Slaanesh did not provide you with a code response."
	killer << "Use the code words in the order provided, during regular conversation, to identify other chaos agents. Whatch out however! You are surrounded by the dead emperor's sheep."
	//End code phrase.
...
/datum/game_mode/proc/equip_traitor(mob/living/carbon/human/traitor_mob, var/safety = 0)
	...
	if (!R)
		traitor_mob << "Unfortunately, Slaanesh wasn't able to get you a radio."
		. = 0
	else
		if (istype(R, /obj/item/device/radio))
			...
			traitor_mob << "Slaanesh has cunningly disguised a Warp Uplink as your [R.name] [loc]. Simply dial the frequency [format_frequency(freq)] to unlock its hidden features."
			...
			traitor_mob << "Slaanesh has cunningly disguised a Warp Uplink as your [R.name] [loc]. Simply enter the code \"[pda_pass]\" into the ringtone select to unlock its hidden features."
			traitor_mob.mind.store_memory("<B>Uplink Passcode:</B> [pda_pass] ([R.name] [loc]).")
	//Begin code phrase.
	if(!safety)//If they are not a rev. Can be added on to.
		traitor_mob << "Slaanesh provided you with the following information on how to identify other agents:"
		if(prob(80))
			traitor_mob << "\red Code Phrase: \black [syndicate_code_phrase]"
			traitor_mob.mind.store_memory("<b>Code Phrase</b>: [syndicate_code_phrase]")
		else
			traitor_mob << "Unfortunetly, Slaanesh did not provide you with a code phrase."
		if(prob(80))
			traitor_mob << "\red Code Response: \black [syndicate_code_response]"
			traitor_mob.mind.store_memory("<b>Code Response</b>: [syndicate_code_response]")
		else
			traitor_mob << "Unfortunately, Slaanesh did not provide you with a code response."
...
/datum/game_mode/proc/equip_exchange(mob/living/carbon/human/mob)
	...
	if (!where)
		mob << "<span class='warning'>Your Chaos contact was unable to send you their secret documents.</span>"
	else
		mob << "<span class='info'>In your [where] is a folder containing <b>secret documents</b> that another chaos group wants. We have set up a meeting with one of their agents on station to make an exchange. Exercise extreme caution as they cannot be trusted and may be hostile.</span>"
		mob.update_icons()