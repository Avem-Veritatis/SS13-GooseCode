...
/mob/living/carbon/proc/swap_hand()
	...
	if(ishuman(src))
		if(!src:has_r_hand())
			hud_used.r_hand_hud_object.icon_state = "template" //A bit of an icon to indicate lack of hand.
		if(!src:has_l_hand())
			hud_used.l_hand_hud_object.icon_state = "template" //I think sacrificing an arm on one of the corruption path is viable.
	if(istype(src.get_active_hand(), /obj/item/weapon/gun))
		src.set_cursor('icons/mob/target.dmi')
	else
		src.set_cursor(null)
	/*if (!( src.hand ))
		src.hands.dir = NORTH
...
*/
		...
		if(TK in mutations)
			item.attack_tk(src) //use telekinesis on a thrown item
...
/mob/living/carbon/proc/get_temperature(var/datum/gas_mixture2/environment)
	var/loc_temp = T0C
	...
	else if(locate(/obj/effect/fake_floor) in get_turf(src)) //Hardcoded room temperature so that these things can be space worthy.
		loc_temp = 295
	...
	//	var/obj/machinery/atmospherics/unary/cryo_cell/C = loc
		...
		//if(C.air_contents.get_pressure() < 10)
		//	loc_temp = environment.temperature
		//else
