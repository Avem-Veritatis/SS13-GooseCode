...
/obj/item/weapon/antag_spawner/borg_tele
	name = "Heretek's Servitor Warp Link"
	desc = "A single-use warp device used to order a Heretek's Servitor."
...
/obj/item/weapon/antag_spawner/borg_tele/attack_self(mob/user as mob)
	if(used)
		user << "The warp device is out of power."
		return
	...
	if(borg_candicates.len > 0)
		...
		user << "<span class='notice'>Unable to connect to our providers. Please wait and try again later or use the teleporter on your uplink to get your points refunded.</span>"
