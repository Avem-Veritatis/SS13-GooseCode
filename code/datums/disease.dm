...
/datum/disease/proc/spread(var/atom/source=null, var/airborne_range = 2,  var/force_spread)
	...
	if(how_spread == DAEMONBORNE) //Daemonborne diseases produce nurgle spirits that spread the disease instead of normal transmission.
		if(prob(1))
			var/mob/living/simple_animal/hostile/retaliate/daemon/poltergeist/nurgle/N = new /mob/living/simple_animal/hostile/retaliate/daemon/poltergeist/nurgle(get_turf(source))
			N.virus_spread = list(src.type)
		return
...
/datum/disease/proc/process()
	...
	if(holder == affected_mob)
		if(affected_mob.stat != DEAD) //he's alive
			if(!src.carrier && "NURGLE" in affected_mob.factions)
				src.carrier = 1
			if(!("NURLGE" in affected_mob.factions) &&  affected_mob.luck > 0 && prob(affected_mob.luck/2)) //If you they lucky enough, don't hit them with disease effects, but still process curing. Also don't cure if they are a nurglite because they wouldn't want to be cured.
				if(has_cure())
					cure()
				return
			stage_act()
