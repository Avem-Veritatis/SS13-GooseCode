...
/mob/living/carbon/monkey/Life()
	...
	var/datum/gas_mixture2/environment // Added to prevent null location errors-- TLE
	if(loc)
		environment = loc.get_air()
...
/mob/living/carbon/monkey
		...
		if(losebreath>0) //Suffocating so do not take a breath
			...
			if(!breath)
				if(istype(loc, /obj/))
					...
					/*
					var/breath_moles = environment.get_pressure()*BREATH_PERCENTAGE
	...
	proc/handle_breath(datum/gas_mixture2/breath)
		...
		if(!breath || (breath.get_pressure() == 0))
			adjustOxyLoss(7)
		...
		if(breath.sleepgas > SA_para_min) // Enough to make us paralysed for a bit
			Paralyse(3) // 3 gives them one second to wake up and run away a bit!
			if(breath.sleepgas > SA_sleep_min) // Enough to make us sleep as well
				sleeping = max(sleeping+2, 10)
		else if(breath.sleepgas > 0.01)	// There is sleeping gas in their lungs, but only a little, so give them a bit of a warning
			if(prob(20))
	...
	proc/handle_environment(datum/gas_mixture2/environment)
		if(!environment)
			return
		//var/environment_heat_capacity = 1 //1. Fuck heat capacity.
		...
		if(!on_fire)
			if((environment.temperature > (T0C + 50)) || (environment.temperature < (T0C + 10)))
				...
				handle_temperature_damage(HEAD, environment.temperature, transfer_coefficient)

		if(stat != 2)
			bodytemperature += 0.01*(environment.temperature - bodytemperature)
