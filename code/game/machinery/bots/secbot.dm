				...
				if(!src.target.handcuffed && !src.arrest_type)
					...
					spawn(60)
						if(get_dist(src, src.target) <= 1)
							...
							playsound(src.loc, pick('sound/voice/bgod.ogg', 'sound/voice/biamthelaw.ogg', 'sound/voice/bsecureday.ogg', 'sound/voice/bradio.ogg', 'sound/voice/binsult.ogg', 'sound/voice/bcreep.ogg'), 100, 0)
...
/obj/machinery/bot/secbot/proc/look_for_perp()
	...
	for (var/mob/living/carbon/C in view(7,src)) //Let's find us a criminal
		...
		if(!src.threatlevel)
			...
			playsound(src.loc, pick('sound/voice/bcriminal.ogg', 'sound/voice/bjustice.ogg', 'sound/voice/bfreeze.ogg'), 100, 0)
