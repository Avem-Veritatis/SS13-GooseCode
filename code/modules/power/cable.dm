...
/obj/structure/cable/heavy/attackby(obj/item/W, mob/user)
	...
	if(istype(W, /obj/item/weapon/wirecutters))
		...
		user.visible_message("\red <b>Sparks fly from the heavy cable as a massive shock is discharged!</b>")

		explosion(get_turf(src),1,2,4,flame_range = 2) //Should delete the cable anyway.
...
/obj/structure/cable/heavy/ex_act(severity)
	switch(severity)
		if(1.0)
			qdel(src)
		if(2.0)
