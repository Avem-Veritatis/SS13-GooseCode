...
/obj/structure/window/temperature_expose(datum/gas_mixture2/air, exposed_temperature, exposed_volume)
...
/*
New Windows for a new age
*/

/obj/structure/window2 //lets do this
	name = "window"
	desc = "A window."
	icon = 'icons/obj/structures.dmi'
	icon_state = "windowlw"
...
/obj/structure/window2/New(location)
	..()
	air_update_turf(1)
	return
...
/obj/structure/window2/bullet_act(var/obj/item/projectile/Proj)
	if((Proj.damage_type == BRUTE || Proj.damage_type == BURN))
		health -= Proj.damage
	..()
	if(health <= 0)
		new /obj/item/weapon/shard(loc)
		new /obj/structure/brokenwindow2(loc)
		qdel(src)
	return

/obj/structure/window2/attackby(obj/item/I, mob/user)
	add_fingerprint(user)
	if(I.damtype == BRUTE || I.damtype == BURN)
		user.changeNext_move(8)
		hit(I.force)
...
/obj/structure/window2/proc/hit(var/damage, var/sound_effect = 1)
	if(reinf) damage *= 0.5
	health = max(0, health - damage)
	if(sound_effect)
		playsound(loc, 'sound/effects/Glasshit.ogg', 75, 1)
...
/obj/structure/brokenwindow2	//this is why we can't have nice things
	name = "broken window"
	desc = "A broken window."
	icon = 'icons/obj/structures.dmi'
	icon_state = "bwindowlw"
	density = 0
	layer = 3.2//Just above doors
	anchored = 1.0
	var/health = 1
