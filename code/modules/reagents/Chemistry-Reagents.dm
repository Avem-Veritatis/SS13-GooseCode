...
/*
			reaction_obj(var/obj/O, var/volume)
				...
				var/datum/gas_mixture2/napalm = new
...
//				M.sdisabilities &= ~1		Replaced by eye surgery
	...
	//			..()		//this was causing hyperzine to be consumed twice...
		...
		/*
		nanites //We have code under this name elsewhere. This causes the drug to near instantly transform you, which is a bit of an issue.
			name = "Nanomachines"
			id = "nanites"
...
//////////////////////////Poison stuff///////////////////////
		...
		toxin/plasma
			...
			on_mob_life(var/mob/living/M as mob)
				...
				return
			/*
			reaction_obj(var/obj/O, var/volume) //For now, lets just hex this out. I may replace the plasma mineral with something completely different anyway.
				src = null                        //And we certainly don't have plasma gas anymore.
...
*/
		cornoil
			...
			reaction_turf(var/turf/simulated/T, var/volume)
				...
				var/hotspot = (locate(/obj/effect/hotspot) in T)
				if(hotspot)
					var/datum/gas_mixture2/lowertemp = T.remove_air( T:air:get_pressure() )
...
//////////////////////////////////////////////The ten friggen million reagents that get you drunk//////////////////////////////////////////////
		...
		gargle_blaster
			...
			id = "gargleblaster"
			description = "Gold bricks and lemons abound."
			reagent_state = LIQUID
			color = "#664300" // rgb: 102, 67, 0
		...
		khornedrink
			name = "Khornate Rum"
			id = "khornedrink"
			description = "The blood god's signature drink."
			reagent_state = LIQUID
			color = "#FF0000"
		...
		tzeenchdrink
			...
			id = "tzeenchdrink"
			description = "The Changer of Way's signature drink."
			reagent_state = LIQUID
			color = "#0000FF"
		...
		slaaneshdrink
			name = "Slaaneshi Shot"
			id = "slaaneshdrink"
			description = "The Lord of Pleasure's signature drink."
			reagent_state = LIQUID
			color = "#FF00FF"
		...
		nurgledrink
			name = "Nurgle's Filth"
			id = "nurgledrink"
			description = "Papa Nurgle's signature drink."
			reagent_state = LIQUID
			color = "#00FF00"
...
*/
		...
		ethanol/rotgut
			name = "Rotgut"
			id = "rotgut"
			description = "That's some high quality rocket fuel that is."
			color = "#664300"
			boozepwr = 10
			...
			on_mob_life(var/mob/living/M as mob)
				if(ishuman(M))
					var/mob/living/carbon/human/H = M
					if(prob(10)) H.suppress_pain += 1 //You can use a ton of rotgut for a surgery!
				M.nutrition += 1
				..()
				return
		...
		ethanol/arcjuice	//I have no idea
			name = "Arc Juice"
			id = "arcjuice"
			description = "I have no idea what this is. Ask Shadow."
			color = "#FFFF91" // rgb: 255, 255, 145
			boozepwr = 25
			var/awarded = 0
			...
			on_mob_life(var/mob/living/M as mob)
				if(ishuman(M))
					var/mob/living/carbon/human/H = M
					if(prob(5)) H.suppress_pain += 1 //You can use a ton of arc juice for a surgery!
				if(!awarded && volume >= 30)
					awarded = 1
					award(M, "What is this even made out of?")
				..()
		...
		ethanol/damassine
			name = "Damassine"
			id = "damassine"
			description = "An alchoholic beverage with a sweet herbal taste and a hint of almond."
			color = "#664340"
			boozepwr = 60
		...
		ethanol/wildsnake
			name = "Wildsnake"
			id = "wildsnake"
			description = "A signature drink of the hive world necromunda."
			color = "#833333"
			boozepwr = 25
		...
		ethanol/tranq
			name = "Tranq"
			id = "tranq"
			description = "A synthetic alchoholic beverage with somewhat unpleasant effects, widely consumed in the Calixis sector."
			color = "#833333"
			boozepwr = 10

			on_mob_life(var/mob/living/M as mob)
				if(prob(2))	M.emote("yawn")
				if(prob(2)) M.eye_blurry += 4
				if(prob(1)) M.drowsyness += 5
				..()
				return
