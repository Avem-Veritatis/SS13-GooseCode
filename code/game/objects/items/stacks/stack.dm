...
/obj/item/stack/Topic(href, href_list)
	...
	if (href_list["make"])
		...
		if (R.one_per_turf && (locate(R.result_type) in usr.loc))
			...
			return
		if (R.on_floor && !istype(usr.loc, /turf/simulated/floor) && !(locate(/obj/effect/fake_floor) in usr.loc)) //If they are "located" on a fake floor they can still build this stuff.
			usr << "\red \The [R.title] must be constructed on the floor!"
			return
		if (R.time)
			usr << "\blue Building [R.title] ..."
			cooldown = 1
			spawn(R.time) cooldown = 0
