...
/mob/living/carbon/human/ork/gretchin/Life()
	set invisibility = 0
	...
	..()
	var/datum/gas_mixture2/enviroment = loc.get_air()
...
/mob/living/carbon/human/ork/gretchin
	...
	proc/orkbreathe()
		...
		if(losebreath>0) //Suffocating so do not take a breath
			...
			breath = orkget_breath_from_internal(BREATH_VOLUME)
		...
		orkhandle_breath(breath)

		if(breath)
			loc.assume_air(breath)


	proc/orkget_breath_from_internal(volume_needed)
		if(internal)
	...
	proc/orkhandle_breath(datum/gas_mixture2/breath)
		if(status_flags & GODMODE)
			return

		if(!breath || (breath.get_pressure() == 0))
			//orks breathe in vaccuum
			return 0
		...
		if(Toxins_pp) // Detect toxins in air
			...
			adjustBruteLoss(breath.poison)
			toxins_alert = max(toxins_alert, 1)

			toxins_used = breath.poison

		else
			toxins_alert = 0

		//Breathe in toxins and out oxygen
		breath.poison -= toxins_used
	...
	proc/orkhandle_chemicals_in_body()
	...
	proc/orkhandle_regular_hud_updates()
