/obj/item/mecha_parts/mecha_equipment/tool/hydraulic_clamp
	name = "power loader"
	desc = "Standard equipment on an imperial sentinel, can be fitted on to many vehichles. Useful for cargo transportation and close combat."
	...
	can_attach(obj/mecha/M as obj)
		if(..())
			if(istype(M, /obj/mecha/combat/sentinel) || istype(M, /obj/mecha/working/ripley))
				return 1
		return 0
...
/obj/item/mecha_parts/mecha_equipment/tool/rcd
	...
	action(atom/target)
		if(istype(target,/area/shuttle)||istype(target, /turf/space/transit) || istype(target, /turf/simulated/wall/necron))//>implying these are ever made -Sieve
			disabled = 1
		else
			disabled = 0
...
/obj/item/mecha_parts/mecha_equipment/generator
	...
	critfail()
		...
		if(prob(10))
			...
			T.visible_message("The [src] suddenly disgorges a cloud of heated promethium.")
			destroy()
		else
			GM.promethium += 5
			GM.temperature = istype(T) ? T.temperature : T20C
			T.visible_message("The [src] suddenly disgorges a cloud of promethium.")
		T.assume_air(GM)
		return
