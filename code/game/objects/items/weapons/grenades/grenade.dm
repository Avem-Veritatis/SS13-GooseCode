	...
	if(!active)
		if(clown_check(user))
			...
			if(iscarbon(user))
				...
				C.throw_mode_on()
			if(user.luck < 0)
				det_time += (user.luck/8)
				if(det_time < 1) det_time = 1
			else
				if(prob(user.luck/2))
					src.throw_range += 1
					src.lucky = 1
...
/obj/item/weapon/grenade/throw_impact(atom/hit_atom)
	..()
	if(lucky == 1)
		spawn(5) prime()

/obj/item/weapon/grenade/attack_self_tk() //Making grenades detonatable with telekinesis. What could possibly go wrong? -Drake
	if(!active)
		if(usr)
			usr << "<span class='warning'>You prime the [name]! [det_time/10] seconds!</span>"
			active = 1
			icon_state = initial(icon_state) + "_active"
			add_fingerprint(usr)
			var/turf/bombturf = get_turf(src)
			var/area/A = get_area(bombturf)
			log_game("[key_name(usr)] has telekinetically primed a [name] for detonation at [A.name] ([bombturf.x],[bombturf.y],[bombturf.z]).")
			spawn(det_time)
				prime()
