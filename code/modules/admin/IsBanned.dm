...
var/global/bancooldown = 0

world/IsBanned(key,address,computer_id)
	if(ckey(key) in admin_datums)

//okay but can we get rid of all this DBquery junk? we're never going to use SQL so why have it?
	...
	if(!guests_allowed && IsGuestKey(key))
		...
		if(!bancooldown)
			...
			bancooldown = 1
			spawn(50) bancooldown = 0
		return list("reason"="guest", "desc"="\nReason: Guests not allowed. Please sign in with a byond account you noob.")
