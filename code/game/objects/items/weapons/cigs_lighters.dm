...
/obj/item/clothing/mask/cigarette/proc/handle_reagents()
	if(iscarbon(loc))
		...
		if (src == C.wear_mask) // if it's in the human/monkey mouth, transfer reagents to the mob
			if(prob(25)) // so it's not an instarape in case of acid //Hm, lets make that twenty five. Acid cigs should be purty bad still.
				reagents.reaction(C, INGEST)
			reagents.trans_to(C, REAGENTS_METABOLISM*1.1) //We want the amount in them to increase faster than metabolism.
			return
...
/obj/item/clothing/mask/cigarette/lho
	name = "\improper Lho Leaf Cigarette"
	desc = "A cigarette infused with the addictive narcotic Lho leaf."
	item_state = "spliffoff"
	icon_state = "spliffoff"
	icon_on = "spliffon"
	icon_off = "spliffoff"
...
/obj/item/clothing/mask/cigarette/lho/New()
	..()
	src.reagents.add_reagent("lho",100) //fills the cig with lho
...
/obj/item/weapon/rollingpaperpack
	...
	desc = "A pack of Kyner Industries rolling papers."
	icon = 'icons/obj/cigarettes.dmi'
