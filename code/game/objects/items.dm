/obj/item
	...
	var/attack_speedmod = 2 //How fast it attacks. Negative numbers speed it up, positive numbers slow it down.
	var/complex_block = 0   //If this weapon may have a block ability.
	var/complex_click = 0   //If control+click can be used for a specific move.
	var/parrying = null     //Parrying code.
	var/can_parry = 0
	var/parryprob = 110
	var/parrycooldown = 2
	var/parryduration = 5
	var/piercingpower = 0 //How much armor it ignores on hit.
	var/list/woundtypes = list(/datum/wound)
	var/list/external_icon = null

/obj/item/proc/handle_block(var/obj/item/I, var/mob/living/user, var/mob/living/target, var/probmod = 0)
	if(can_parry)
		if(src.force >= 10 && parrying == user)                                                  //If force is less than this level, that probably means it is some kind of inactive blade, and can't be used to parry.
			if(prob((parryprob - I.force)+probmod) && user.dir != target.dir && !target.lying)               //Not the most elegant solution but I don't want to have to track multiple different variables scattered around objects.
				user.visible_message("<span class ='danger'>[target] has parried [user]'s attack!</span>")
				parrying = null
				return 0
			else
				target << "\red You fail to block the [I]!"
		else if(src.force >= 10 && parrying == null && target.dir == get_dir(target, user) && prob((parryprob - I.force)+probmod)/2)
			user.visible_message("<span class ='danger'>[target] has parried [user]'s attack!</span>")
			return 0
		else if(src.force >= 10 && parrying == null && user.dir != target.dir && prob((parryprob - I.force)+probmod)/6)
			user.visible_message("<span class ='danger'>[target] has parried [user]'s attack!</span>")
			return 0
	return 1

/obj/item/proc/handle_ctrlclick(var/mob/living/user, var/mob/living/target)
	if(can_parry)
		if(user.next_click <= world.time && !parrying)
			user << "\red You prepare to parry a blow from [target]."
			parrying = target
			spawn(parryduration*10) parrying = null
			user.next_click += parrycooldown
