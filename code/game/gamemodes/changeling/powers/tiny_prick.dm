...
/obj/effect/proc_holder/changeling/sting/proc/set_sting(var/mob/user)
	user << "<span class='notice'>You prepare a poison, use alt+click or middle mouse button on target to sting them.</span>"
...
/obj/effect/proc_holder/changeling/sting/proc/unset_sting(var/mob/user)
	user << "<span class='warning'>You put your poisons away, you can't poison anyone for now.</span>"
...
/obj/effect/proc_holder/changeling/sting/can_sting(var/mob/user, var/mob/target)
	if(!..())
		return
	if(!user.mind.changeling.chosen_sting)
		user << "You haven't prepared your tools yet!"
	if(!iscarbon(target))
		return
	if(!isturf(user.loc))
		return
...
/obj/effect/proc_holder/changeling/sting/sting_feedback(var/mob/user, var/mob/target)
	if(!target)
		return
	user << "<span class='notice'>You stealthily poison [target.name].</span>"
	if(target.mind && target.mind.changeling)
...
/obj/effect/proc_holder/changeling/sting/transformation
	...
	desc = "You silently sting a human, injecting a retrovirus that forces them to transform."
...
/obj/effect/proc_holder/changeling/sting/transformation/can_sting(var/mob/user, var/mob/target)
	...
	if((HUSK in target.mutations) || !check_dna_integrity(target))
		user << "<span class='warning'>Your sting appears ineffective against its DNA.</span>"
		return 0
	return 1
...
/obj/effect/proc_holder/changeling/sting/transformation/sting_action(var/mob/user, var/mob/target)
	...
	if(ismonkey(target))
		user << "<span class='notice'>You stealthily sting [target.name].</span>"
...
obj/effect/proc_holder/changeling/sting/extract_dna
	...
	desc = "You stealthily sting a target and extract their DNA."
...
obj/effect/proc_holder/changeling/sting/mute
	name = "Mute Sting"
	desc = "You silently poison a human, completely silencing them for a short time."
...
obj/effect/proc_holder/changeling/sting/blind
	...
	helptext = "This poison completely blinds a target for a short time. The target does not notice they have been stung."
...
obj/effect/proc_holder/changeling/sting/LSD
	name = "Hallucination poison"
	desc = "Causes terror in the target."
	helptext = "You evolve the ability to poison a target with a powerful hallucinogenic chemical. The target does not notice they have been stung.  The effect occurs after 30 to 60 seconds."
...
obj/effect/proc_holder/changeling/sting/cryo
	...
	desc = "You silently poison a human with a cocktail of chemicals that freeze them."
