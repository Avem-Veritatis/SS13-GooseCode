...
datum/controller/game_controller
	...
	var/mob_processing_killed = 0
	var/sun_processing_killed = 0
	var/machine_processing_killed = 0
	var/garbage_processing_killed = 0
	var/powernets_processing_killed = 0
	var/objects_processing_killed = 0
...
datum/controller/game_controller/New()
	...
	//for(var/i=0, i<max_secret_rooms, i++) //Emperor... No wonder we have been having problems. Hexing this out now.
	//	set_up_all_ruins()
...
datum/controller/game_controller/proc/setup_objects() //Server should start up a lot faster without an air master. -Drake
	world << "\red \b Pruning feathers..."
	for(var/atom/movable/object in world)
		object.initialize()

	world << "\red \b Eating bread..."
	//for(var/obj/machinery/atmospherics/machine in world)
	//	machine.build_network()
	/*
	for(var/area/AR in world)
		if(istype(AR, /area/maintenance/undermaint) || istype(AR, /area/maintenance/safe))
			for(var/turf/T in get_area_turfs(AR.type))
				if(prob(75)) //I got a better idea!
					new /obj/effect/warp/shadow(T) //I won't let ghosts simply meta everything down here. That would be no fun.
	*/

	world << "\red \b Reading heretical books..."
	...
	//		T.broadcast_status()
	world << "\red \b Guarding doors in Canada..."
	world << "\red \b Goosecode Version 43 is ready..."
		...
		while(1)	//far more efficient than recursively calling ourself
			...
			if(processing)
				...
				*/
				...
				//POWERNETS
				if(!powernets_processing_killed)
					timer = world.timeofday
					process_powernets()
