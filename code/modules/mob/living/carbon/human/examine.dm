/*
TODO: Make this be more informative, especially in regard to injuries
Also, using the macro for \icon[usr] could be interesting here
*/
...
/mob/living/carbon/human/examine()
	...
	var/temp = getCloneLoss() //no need to calculate each of these twice

	msg += "<span class='warning'>"

	for(var/obj/item/organ/limb/O in src.organs) //Better flavor text for these types of injury!
		var/list/alreadysaid = list()
		for(var/datum/wound/W in O.wounds)
			if(!(W.desc in alreadysaid))
				alreadysaid.Add(W.desc)
				msg += "[t_his] [O.getDisplayName()] [W.desc].\n"
		for(var/scar in O.scars)
			if(!(scar in alreadysaid))
				alreadysaid.Add(scar)
				msg += "[t_his] [O.getDisplayName()] [scar].\n"
