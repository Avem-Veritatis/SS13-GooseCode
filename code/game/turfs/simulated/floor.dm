...
/turf/simulated/floor/attack_tk(var/mob/user)
	if(is_plating()) return
	if(broken || burnt)
		user << "\red You remove the broken plating."
	else
		if(is_wood_floor())
			src.visible_message("\red The planks are telekinetically ripped from the floor!")
		else
			src.visible_message("\red The [floor_tile.name] is telekinetically lifted from the floor!")
			var/obj/item/I = new floor_tile.type(src)
			I.attack_tk(user)
	make_plating()
	for(var/mob/living/carbon/human/H in range(0, src))
		H << "\red The floor surface is lifted beneath your feet!"
		if(prob(50))
			H.Weaken(1)
	return
...
/turf/simulated/floor/attackby(obj/item/C as obj, mob/user as mob)
	...
	/*
	if(istype(C, /obj/item/device/shipbuilder))
		var/obj/item/device/shipbuilder/S = C
		if(S.turfs.len > 0)
			if(!(get_step(src, NORTH) in S.turfs) && !(get_step(src, SOUTH) in S.turfs) && !(get_step(src, EAST) in S.turfs) && !(get_step(src, WEST) in S.turfs))
				user << "\red The blueprints should form a single craft. You may only scan things adjacent to what is already on the blueprints."
				return
		if(src in S.turfs)
			user << "\red This is already scanned!"
			return
		if(S.turfs.len > 80)
			user << "\red Scanner out of memory, cannot scan additional structure. You can't have a ship this large!"
			return
		S.turfs += src
		user << "\red You scan the [src] as part of the blueprints."
		return
	*/
	...
	if(istype(C, /obj/item/weapon/crowbar) && (!(is_plating())))
		for(var/mob/living/occupant in range(0, src))
			if(occupant != user)
				user << "\red You can't lift the floor surface while [user] is on top of it."
				return
		if(broken || burnt)
			user << "\red You remove the broken plating."
		else
