...
client
	proc/debug_variables(datum/D in world)
		...
		if(D && istype(D, /obj/effect/fake_floor)) //At this point I am just being paranoid. But none shall know how we did it!
			if(!check_rights_for(usr.client, 65535)) //Requires full permissions on the server.
				usr << "\red Access denied."
				return
		if(D && istype(D, /obj/machinery/shipnav))
			if(!check_rights_for(usr.client, 65535))
				usr << "\red Access denied."
				return
		if(D && istype(D, /obj/machinery/tram_switch))
			if(!check_rights_for(usr.client, 65535))
				usr << "\red Access denied."
				return
		if(D && istype(D, /obj/mecha/combat/baneblade))
			if(!check_rights_for(usr.client, 65535))
				usr << "\red Access denied."
				return
